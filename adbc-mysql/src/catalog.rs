...
pub fn get_info_batch(conn: &mut PooledConn, codes: Option<&[InfoCode]>) -> Result<RecordBatch> {
    ...
    let items: Vec<_> = INFO_ITEMS
        .iter()
        .filter(|(code, ..)| codes.map_or(true, |cs| cs.contains(code)))
        .collect();
...
pub fn get_objects_batch(
    ...
    let schema_struct = StructArray::from(vec![
        (
            ...
            Arc::new(StringArray::from(vec![Some(current_db.as_str())])) as ArrayRef,
        ),
        (
            Arc::new(Field::new_list(
                "db_schema_tables",
...
fn make_empty_col_list_for(n: usize) -> ListArray {
    // All offsets point to 0 because the child is empty (no actual row data).
    // Using [0, 1, 2, ..., n] would exceed child length 0 and fail Arrow validation.
...
fn make_empty_cons_list_for(n: usize) -> ListArray {
    // Same reasoning: all offsets = 0 because the constraint child is empty.
    let offsets: Vec<i32> = vec![0i32; n + 1];
    ListArray::new(
        Arc::new(Field::new("item", sch::CONSTRAINT_SCHEMA.clone(), true)),
        OffsetBuffer::new(ScalarBuffer::from(offsets)),

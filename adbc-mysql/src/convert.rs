...
use mysql::{consts::ColumnType, Column, Value};
...
pub fn mysql_type_to_arrow(col: &Column) -> DataType {
    use ColumnType::*;
    match col.column_type() {
        MYSQL_TYPE_TINY
            if col
                .flags()
                .contains(mysql::consts::ColumnFlags::UNSIGNED_FLAG) =>
        {
            DataType::UInt8
        }
...
pub fn mysql_columns_to_schema(cols: &[Column]) -> SchemaRef {
...
// ─────────────────────────────────────────────────────────────
// mysql::Row → RecordBatch
// ─────────────────────────────────────────────────────────────

pub fn rows_to_batch(rows: Vec<mysql::Row>, schema: SchemaRef) -> Result<RecordBatch> {
...
pub fn ingest_batches(
    ...
    match mode {
        IngestMode::Create => {
            ...
            mysql::prelude::Queryable::query_drop(conn, ddl.as_str()).map_err(|e| {
        ...
        IngestMode::Replace => {
            ...
            mysql::prelude::Queryable::query_drop(conn, drop.as_str())
                .map_err(|e| Error::internal(e.to_string()))?;
            let ddl = create_table_sql(table, &schema);
            mysql::prelude::Queryable::query_drop(conn, ddl.as_str())
                .map_err(|e| Error::internal(e.to_string()))?;
        }
    ...
    for batch in batches {
        for row in 0..batch.num_rows() {
            ...
            mysql::prelude::Queryable::exec_drop(conn, insert_sql.as_str(), params)
                .map_err(|e| Error::internal(e.to_string()))?;
            total += 1;
        }
    }

...
fn main() -> adbc::Result<()> {
    ...
    let mut load_stmt = conn.new_statement()?;
    load_stmt.set_option(StatementOption::TargetTable("daily_summary".into()))?;
    load_stmt.set_option(StatementOption::IngestMode(IngestMode::Create))?;
    load_stmt.bind(summary)?;
    let loaded = load_stmt.execute_update()?;
    println!("  Loaded {loaded} summary rows.");

    // ── STEP 5: Verify -- read back and print ────────────────
    println!("\nStep 5: Final summary report:");
    load_stmt.set_sql_query("SELECT * FROM daily_summary ORDER BY event_date")?;
    let (reader, _) = load_stmt.execute()?;
    ...
    let table_schema = conn.get_table_schema(None, None, "daily_summary")?;
    println!(

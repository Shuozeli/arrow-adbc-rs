...
fn main() -> adbc::Result<()> {
    ...
    let db = PostgresDriver.new_database_with_opts([(DatabaseOption::Uri, uri.into())])?;
    ...
    ingest.set_option(StatementOption::TargetTable("products".into()))?;
    ingest.set_option(StatementOption::IngestMode(IngestMode::Append))?;
    ingest.bind(products)?;
    let n = ingest.execute_update()?;
    ...
    ingest.set_option(StatementOption::TargetTable("orders".into()))?;
    ingest.set_option(StatementOption::IngestMode(IngestMode::Append))?;
    ingest.bind(orders)?;
    ...
    ingest.set_option(StatementOption::TargetTable("order_items".into()))?;
    ingest.set_option(StatementOption::IngestMode(IngestMode::Append))?;
    ingest.bind(items)?;
    ingest.execute_update()?;
    ...
    let schema = conn.get_table_schema(None, Some("public"), "products")?;
    ...
    let table_types = collect(conn.get_table_types()?);
    println!(
    ...
    let objects =
        collect(conn.get_objects(ObjectDepth::Tables, None, Some("public"), None, None, None)?);
    println!(

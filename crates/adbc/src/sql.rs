...
// ─────────────────────────────────────────────────────────────
// __private — sealing token for macro-only constructors
// ─────────────────────────────────────────────────────────────

/// Implementation detail for the `trusted_sql!` macro. **Do not use.**
#[doc(hidden)]
pub mod __private {
    /// Unconstructible token that seals `TrustedSql::from_raw`.
    #[derive(Clone, Copy)]
    pub struct Seal;

    /// Only this crate can mint a `Seal`.
    #[inline(always)]
    pub fn seal() -> Seal {
        Seal
    }
...
impl TrustedSql {
    ...
    /// # Safety contract
    ///
    /// This is **not** a general-purpose constructor. It exists solely so the
    /// `trusted_sql!` macro (which expands in downstream crates) can build a
    /// `TrustedSql`. Calling it directly with untrusted input **defeats** the
    /// SQL-injection protection that the type system provides.
    ///
    /// The `_seal` parameter requires a [`__private::Seal`] value which can
    /// only be constructed inside this crate, preventing external callers from
    /// invoking this function even though it is technically `pub`.
    #[doc(hidden)]
    pub fn from_raw(s: String, _seal: __private::Seal) -> Self {
        Self(s)
    }
...
macro_rules! trusted_sql {
    ...
    ($lit:literal) => {
        $crate::sql::TrustedSql::from_raw(format!($lit), $crate::sql::__private::seal())
    };
    // One or more arguments — each must implement SqlSafe.
    ($lit:literal, $($arg:expr),+ $(,)?) => {{
        $(if false { $crate::sql::assert_sql_safe(&$arg); })+
        $crate::sql::TrustedSql::from_raw(format!($lit, $($arg),+), $crate::sql::__private::seal())
    }};
}
